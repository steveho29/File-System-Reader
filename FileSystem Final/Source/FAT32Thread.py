...
class DFSThread(QThread):
    ...
    def DFS_SDET(self, rootData, sectors, space=0):
        fp = open(self.diskPath, 'rb')
        fp.seekable()
        ...
        for i, sector in enumerate(sectors):
            ...
            for j in range(int(self.bytesPerSector/32)):
                entry = fp.read(32)
                status = unpack('B', entry[0:1])[0]
                if status == 229 or status == 0:
                    continue
                ...
                fileOrder += 1
                mainEntry['Type'] = 'Folder' if mainEntry['Status'] == 16 else 'File'

                self.fileSignal.emit(mainEntry.copy())

                if mainEntry['Status'] == 16:
                    self.DFS_SDET(mainEntry, subFileSectors, space)

    def run(self):
        self.DFS_SDET(self.rootFolder, self.rootFolder['Sectors'])
